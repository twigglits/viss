...
if [ "$ALL_MATCH" = true ]; then
    ...
    # Compare against reference values in stat_out_base_500.json
    echo -e "\n${BLUE}=== Comparing Actual Counts Against Expected Counts ===${NC}"
    
    if [ -f "stat_out_base_500.json" ]; then
        ...
        for event in $(echo "${!all_events[@]}" | tr ' ' '\n' | sort); do
            ...
            if command -v jq &> /dev/null; then
                ...
                reference_count=$(jq -r ".\"$event\"?.expected // 0" stat_out_base_500.json)
            else
                # Fallback to grep/sed for systems without jq
                reference_count=$(grep -o "\"$event\".*expected.*[0-9]\+" stat_out_base_500.json | grep -o '[0-9]\+' | head -1)
        ...
        if [ "$REFERENCE_CHECK_PASS" = true ]; then
            echo -e "\n${GREEN}PASS:${NC} All event counts match reference values in stat_out_base_500.json"
        else
            echo -e "\n${RED}WARNING:${NC} Some event counts differ from reference values in stat_out_base_500.json"
            # Don't exit with error yet - this is just a warning at this point
            echo -e "${BLUE}Note: This indicates potential changes in behavior. Update stat_out_base_500.json if these changes are expected.${NC}"
        fi
    else
        echo -e "${BLUE}Reference file stat_out_base_500.json not found. Skipping reference comparison.${NC}"
    fi
