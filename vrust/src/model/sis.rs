...
impl SisState {
    pub fn new_zero(cfg: &SisConfig) -> Self {
        Self {
            y: vec![0.0; cfg.state_size()],
        }
    }

    pub fn init_from_seeding(cfg: &SisConfig, seeding_per_age: &[f64]) -> Self {
        let mut s = Self::new_zero(cfg);
        for a in 0..cfg.n_age {
            let n = cfg.pop[a];
            let seed = seeding_per_age[a].min(n.max(0.0));
            let (idx_s, i0) = indices(cfg, a);
            s.y[idx_s] = n - seed;
            s.y[i0] = seed;
        }
        s
...
fn indices(cfg: &SisConfig, a: usize) -> (usize, usize) {
    let block = 1 + cfg.k_i;
    let base = a * block;
    let idx_s = base;
    let i0 = idx_s + 1;
...
impl SisModel {
    pub fn new(cfg: SisConfig) -> anyhow::Result<Self> {
        cfg.check()?;
        Ok(Self { cfg })
    }
    ...
    pub fn deriv(&self, t: f64, y: &[f64], dy: &mut [f64]) {
        ...
        for a in 0..cfg.n_age {
            ...
            let recovered = ki_gamma * y[i0 + cfg.k_i - 1];
            dy[s_idx] += recovered;
        }
    ...
    pub fn simulate(&self, state: &mut SisState, t0: f64, t_end: f64, dt: f64) -> Vec<(f64, Vec<f64>)> {
        let mut t = t0;
        let mut out = Vec::new();
