...
TEST(Condom, TestCondom) {
    ...
    while (std::getline(cfg_in, line)) {
        if (line.find("condom.enabled = false") != std::string::npos) {
            cfg_out << "condom.enabled = true\n";
        } else {
            cfg_out << line << "\n";
        }
    }
